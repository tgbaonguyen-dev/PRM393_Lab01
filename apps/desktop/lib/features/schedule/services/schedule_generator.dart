...
class ScheduleGenerator {
  ...
  static List<ClassLesson> generate({
    ...
    if (!rule.weekdays.contains(normalizedFirstDate.weekday)) {
      throw ArgumentError('Ngày bắt đầu phải thuộc đúng cặp thứ của mã lịch $scheduleCode.');
    }
    ...
    while (lessons.length < lessonCount) {
      if (rule.weekdays.contains(cursor.weekday)) {
        ...
        lessons.add(ClassLesson(
          lessonId: '$classOfferingId-L${sequence.toString().padLeft(2, '0')}',
  ...
  static List<ClassLesson> replaceLessonDate({
    ...
    if (lessons.any((lesson) =>
        lesson.sequenceNumber != sequenceNumber && _sameDate(lesson.date, normalizedDate))) {
      throw ArgumentError('Ngày đã được sử dụng cho một buổi học khác.');
    }

    final index = lessons.indexWhere((lesson) => lesson.sequenceNumber == sequenceNumber);
    ...
    if (index < lessons.length - 1 && !normalizedDate.isBefore(lessons[index + 1].date)) {
      throw ArgumentError('Ngày mới phải trước buổi ${sequenceNumber + 1}.');
    }

    final updated = List<ClassLesson>.of(lessons);
    updated[index] = updated[index].copyWith(date: normalizedDate, isAdjusted: true);
    return updated;
  }
